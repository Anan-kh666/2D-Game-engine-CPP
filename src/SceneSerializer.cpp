...
void SceneSerializer::Serialize(const std::string &filepath) {
  ...
  for (auto [entityID] :
       m_Scene->GetRegistry().storage<entt::entity>().each()) {
    json entityData;
    ...
    if (m_Scene->GetRegistry().all_of<Rigidbody2DComponent>(entityID)) {
      ...
      entityData["Rigidbody2DComponent"] = {
          {"Type", (int)rb2d.Type}, {"FixedRotation", rb2d.FixedRotation}};
    }
...
bool SceneSerializer::Deserialize(const std::string &filepath) {
  ...
  for (auto &entityData : sceneData["Entities"]) {
    ...
    if (entityData.contains("TransformComponent")) {
      ...
      tc.Position = {entityData["TransformComponent"]["Position"][0],
                     entityData["TransformComponent"]["Position"][1],
                     entityData["TransformComponent"]["Position"][2]};
      tc.Size = {entityData["TransformComponent"]["Size"][0],
                 entityData["TransformComponent"]["Size"][1]};
    }
    if (entityData.contains("SpriteRendererComponent")) {
      auto &src =
          m_Scene->GetRegistry().emplace<SpriteRendererComponent>(entity);
      src.Color = {entityData["SpriteRendererComponent"]["Color"][0],
    ...
    if (entityData.contains("BoxCollider2DComponent")) {
      auto &bc2d =
          m_Scene->GetRegistry().emplace<BoxCollider2DComponent>(entity);
      bc2d.Size = {entityData["BoxCollider2DComponent"]["Size"][0],
  ...
  std::cout << "Successfully loaded scene from " << filepath << "!"
            << std::endl;
  return true;
}